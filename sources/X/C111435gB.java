package X;

import com.obwhatsapp.payments.ui.mapper.register.IndiaUpiMapperPendingActivity;

/* renamed from: X.5gB  reason: invalid class name and case insensitive filesystem */
public abstract class C111435gB extends C14530pL {
    public boolean A00 = false;

    public C111435gB() {
        C110105dW.A0t(this, 103);
    }

    public void A1q() {
        if (!this.A00) {
            this.A00 = true;
            IndiaUpiMapperPendingActivity indiaUpiMapperPendingActivity = (IndiaUpiMapperPendingActivity) this;
            C49132Rg r3 = (C49132Rg) C110115dX.A08(this);
            C16150sX r1 = r3.A1s;
            C14550pN.A15(r1, indiaUpiMapperPendingActivity);
            C14530pL.A0b(r3, r1, indiaUpiMapperPendingActivity, C110105dW.A0F(r1));
            indiaUpiMapperPendingActivity.A00 = C110105dW.A0O(r1);
        }
    }
}
